...
AS
  ...
  BEGIN
	...
	WHERE  p.OrderTypeId = ob.OrderType and p.StartDateForClient = 1) >= @FilterAcceptDateBeg  
	AND    	
	(SELECT top 1 CAST(h.ChangeDateTime AS DATE)
	FROM dbo.OrderPipelineSteps p 
	INNER JOIN dbo.OrderStatusHistory h
	ON h.OldStatus = p.FromStatus  and h.OrderId = ob.Id
	WHERE  p.OrderTypeId = ob.OrderType and p.StartDateForClient = 1) <= @FilterAcceptDateEnd
	AND (exists(SELECT *  
