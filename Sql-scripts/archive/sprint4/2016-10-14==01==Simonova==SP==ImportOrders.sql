...
@OrderDate datetime2(7),	   @СonfirmedDate datetime2(7),	     @AcceptedDate datetime2(7),
...
if not exists (SELECT Id FROM [dbo].[AspNetUsers] WHERE LTRIM(RTRIM(UPPER([DisplayName]))) = LTRIM(RTRIM(UPPER(@CreatedByUser))))
	...
	if (@PriotityType = '???????? ??????') set @PriotityTypeId = 0
	...
	SELECT @OrdersBaseId = OrdersBaseId from @table
		...
		BEGIN	
		 ...
		 IF (LTRIM(RTRIM(UPPER(@TripType))) = '?????????') set @TripTypeId = 0
		 ELSE IF (LTRIM(RTRIM(UPPER(@TripType))) = '?????????????') set @TripTypeId = 1
	...
	WHERE LTRIM(RTRIM(UPPER([Name]))) = LTRIM(RTRIM(UPPER(@ToCountry)))
	 ...
	 IF (LTRIM(RTRIM(UPPER(@NeedReturn))) = 'ДА') SET @NeedReturnId = 1
	 ELSE SET @NeedReturn = 0
